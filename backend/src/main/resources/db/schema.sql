...
DROP TABLE IF EXISTS `lab_open_rule`;
...
CREATE TABLE `department` (
    ...
    `leader_name` VARCHAR(50) DEFAULT NULL COMMENT 'Leader name',
...
CREATE TABLE `sys_user` (
    ...
    `user_no` VARCHAR(50) DEFAULT NULL COMMENT 'Student or job number',
...
CREATE TABLE `lab` (
    ...
    `lab_type` VARCHAR(50) DEFAULT NULL COMMENT 'Lab type',
...
CREATE TABLE `lab_open_rule` (
    ...
    `max_reservation_hours` INT NOT NULL DEFAULT 4 COMMENT 'Max reservation hours',
    `status` TINYINT NOT NULL DEFAULT 1 COMMENT '1 enabled, 0 disabled',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Created time',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Updated time',
    PRIMARY KEY (`id`),
    KEY `idx_lab_open_rule_lab_id` (`lab_id`),
    CONSTRAINT `fk_lab_open_rule_lab`
        FOREIGN KEY (`lab_id`) REFERENCES `lab` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Lab open rule table';
...
CREATE TABLE `lab_device` (
    ...
    `model_no` VARCHAR(100) DEFAULT NULL COMMENT 'Model',
...
CREATE TABLE `lab_reservation` (
    ...
    `course_or_project_name` VARCHAR(100) DEFAULT NULL COMMENT 'Course or project name',
    ...
    KEY `idx_lab_reservation_reservation_date` (`reservation_date`),
    KEY `idx_lab_reservation_time` (`lab_id`, `start_time`, `end_time`),
